...
namespace MD5
{
	...
	public class MD5
	{
		...
		/// </summary>
        protected BitVector[] X = new BitVector[16];	
		...
		public string Value
		{
			...
			set
			{
                ByteInput = new BitVector[value.Length];
				for (int i =0; i<value.Length;i++)
                    ByteInput[i] = (BitVector)(byte)value[i];
	    ...
	    /// the input bytes
	    /// </summary>
	    public BitVector[] ByteInput
	    {
	        get { return _byteInput; }
	        set { _byteInput = value; }
	    }
		...
		/// </summary>
        protected void TransH(ref BitVector a, BitVector b, BitVector c, BitVector d, uint k, ushort s, uint i)
		{
            a = b + (a + (b ^ c ^ d) + X[k] + (BitVector)T[i - 1]).RotateLeft(s);
		}
